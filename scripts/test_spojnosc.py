...
sprawdz("LEVEL_HOLD_MIN", num(r"\bLEVEL_HOLD_MIN\s*=\s*(\d+)", ENGINE, "hold js"),
        num(r"\nLEVEL_HOLD_MIN\s*=\s*(\d+)", CONFIG, "hold py"))
sprawdz("ETA_BUFFER", num(r"ETA_BUFFER_MIN\s*=\s*([\d.]+)", ENGINE, "eta buffer js"),
        num(r"NEPTUN_ETA_BUFFER_MIN\s*=\s*([\d.]+)", CONFIG, "eta buffer py"))
sprawdz("ETA_ELEVATED", num(r"ETA_ELEVATED_MIN\s*=\s*([\d.]+)", ENGINE, "eta yellow js"),
        num(r"NEPTUN_ETA_ELEVATED_MIN\s*=\s*([\d.]+)", CONFIG, "eta yellow py"))
sprawdz("ETA_HIGH", num(r"ETA_HIGH_MIN\s*=\s*([\d.]+)", ENGINE, "eta red js"),
        num(r"NEPTUN_ETA_HIGH_MIN\s*=\s*([\d.]+)", CONFIG, "eta red py"))
sprawdz("ETA_MIN_SOURCES", num(r"ETA_MIN_SOURCES\s*=\s*([\d.]+)", ENGINE, "eta src js"),
        num(r"NEPTUN_ETA_MIN_SOURCES\s*=\s*([\d.]+)", CONFIG, "eta src py"))
...
# ── korroboracja: cała klasa RSS NIE może sama osiągnąć progu ─────────────────
...
if _th is not None and _media_cap is not None and _media_cap >= _th:
    bledy.append(f"cap media={_media_cap} ≥ próg {_th}: same artykuły RSS mogą zaalarmować")
